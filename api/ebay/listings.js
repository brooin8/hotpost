export default async function handler(req, res) {
  ...
  try {
    ...
    try {
        ...
        if (inventoryData.inventoryItems && inventoryData.inventoryItems.length > 0) {
          const transformedListings = {
            itemSummaries: inventoryData.inventoryItems.map(item => {
              ...
              if (images.length === 0) {
                images.push('https://via.placeholder.com/400x400/eeeeee/666666?text=No+Image');
              }
              
              // Log image count for debugging
              if (images.length > 1) {
                console.log(`📸 Inventory item ${item.sku} has ${images.length} images`);
              }
              ...
              return {
                ...
                imageCount: images.length // For debugging
              };
            }),
      ...
      // Try 2: Get active seller listings and their full descriptions via Trading API
      ...
      // Step 2a: First get list of active items with GetMyeBaySelling
      const getListingsBody = `<?xml version="1.0" encoding="utf-8"?>
        <GetMyeBaySellingRequest xmlns="urn:ebay:apis:eBLBaseComponents">
          ...
          <ActiveList>
            ...
            <Pagination>
              <EntriesPerPage>${Math.min(parseInt(limit), 50)}</EntriesPerPage>
      ...
      try {
        const listingsResponse = await fetch(tradingApiUrl, {
          method: 'POST',
          headers: {
          ...
          body: getListingsBody
        });
        
        if (listingsResponse.ok) {
          const xmlText = await listingsResponse.text();
          console.log('✅ eBay GetMyeBaySelling API Response received:', xmlText.substring(0, 500));
          
          // Check for errors first
          if (xmlText.includes('<Errors>')) {
            console.log('❌ GetMyeBaySelling API returned errors:', xmlText.substring(0, 1000));
          } else {
            // Extract ItemIDs from the listings response
            const itemIdMatches = xmlText.match(/<ItemID>(.*?)<\/ItemID>/g) || [];
            const itemIds = itemIdMatches.map(match => match.replace(/<\/?ItemID>/g, ''));
            
            console.log(`📋 Found ${itemIds.length} active item IDs:`, itemIds.slice(0, 5));
            
            if (itemIds.length > 0) {
              // Step 2b: Get full details for each item using GetItem calls
              console.log(`🔍 Fetching full descriptions for ${Math.min(itemIds.length, 10)} items...`);
              
              const itemDetailsPromises = itemIds.slice(0, 10).map(async (itemId, index) => {
                ...
                try {
                  const itemResponse = await fetch(tradingApiUrl, {
                    method: 'POST',
                    headers: {
                      'Content-Type': 'text/xml; charset=utf-8',
                      'X-EBAY-API-COMPATIBILITY-LEVEL': '1193',
                  ...
                  if (itemResponse.ok) {
                    ...
                    if (itemXml.includes('<Errors>')) {
                      console.log(`❌ GetItem failed for ${itemId}:`, itemXml.substring(0, 500));
                      return null;
                    }
                    
                    // Extract detailed info from GetItem response
                    ...
                    // Extract FULL DESCRIPTION - this is the key part!
                    let fullDescription = '';
                    const descriptionMatch = itemXml.match(/<Description><\!\[CDATA\[([\s\S]*?)\]\]><\/Description>/);
                    if (descriptionMatch) {
                      fullDescription = descriptionMatch[1];
                    } else {
                      const simpleDescriptionMatch = itemXml.match(/<Description>([\s\S]*?)<\/Description>/);
                      if (simpleDescriptionMatch) {
                        fullDescription = simpleDescriptionMatch[1];
                      }
                    }
                    
                    // Extract subtitle
                    const subtitle = (itemXml.match(/<SubTitle><\!\[CDATA\[(.*?)\]\]><\/SubTitle>/) || itemXml.match(/<SubTitle>(.*?)<\/SubTitle>/) || [])[1] || '';
                    
                    // Create comprehensive description
                    const comprehensiveDescription = [subtitle, fullDescription].filter(Boolean).join('\n\n') || `Active eBay listing: ${title.substring(0, 100)}...`;
                    
                    console.log(`📄 Item ${itemId} full description extracted:`, {
                      hasSubtitle: !!subtitle,
                      hasFullDescription: !!fullDescription,
                      fullDescriptionLength: fullDescription.length,
                      descriptionPreview: fullDescription.substring(0, 200) + '...'
                    });
                    
                    // Extract ALL images from multiple eBay API fields
                    const images = [];
                    const imageUrls = new Set(); // Use Set to avoid duplicates
                    
                    // Method 1: Extract from PictureDetails > PictureURL (most common)
                    const pictureMatches = itemXml.match(/<PictureURL>(.*?)<\/PictureURL>/g) || [];
                    pictureMatches.forEach(match => {
                      const url = match.replace(/<\/?PictureURL>/g, '').trim();
                      if (url && url.startsWith('http')) {
                        imageUrls.add(url);
                      }
                    });
                    
                    // Method 2: Extract from PictureDetails section specifically
                    const pictureDetailsMatch = itemXml.match(/<PictureDetails>[\s\S]*?<\/PictureDetails>/);
                    if (pictureDetailsMatch) {
                      const pictureSection = pictureDetailsMatch[0];
                      const allPictureUrls = pictureSection.match(/<PictureURL>(.*?)<\/PictureURL>/g) || [];
                      allPictureUrls.forEach(match => {
                        const url = match.replace(/<\/?PictureURL>/g, '').trim();
                        if (url && url.startsWith('http')) {
                          imageUrls.add(url);
                        }
                    ...
                    // Method 3: Extract from ExternalPictureURL
                    const externalPictureMatches = itemXml.match(/<ExternalPictureURL>(.*?)<\/ExternalPictureURL>/g) || [];
                    externalPictureMatches.forEach(match => {
                      const url = match.replace(/<\/?ExternalPictureURL>/g, '').trim();
                      if (url && url.startsWith('http')) {
                        imageUrls.add(url);
                      }
                    });
                    
                    // Method 4: Add GalleryURL if available (usually the main image)
                    if (galleryURL && galleryURL.startsWith('http')) {
                      imageUrls.add(galleryURL);
                    }
                    
                    // Convert Set to Array and create image objects with proxied URLs for mobile compatibility
                    const uniqueImageUrls = Array.from(imageUrls);
                    uniqueImageUrls.forEach((url, index) => {
                      const highQualityUrl = enhanceEbayImageQuality(url);
                      const proxiedUrl = `/api/proxy/image?url=${encodeURIComponent(highQualityUrl)}`;
                      images.push({
                        url: proxiedUrl, // Use proxied URL for mobile compatibility
                        originalUrl: url, // Keep original for debugging
                        directUrl: highQualityUrl, // Direct enhanced URL as fallback
                        thumbnailUrl: url.includes('$_') ? url : url + '?set_id=8800005008', // Keep thumbnail for quick preview
                    ...
                    if (images.length === 0) {
                      images.push({
                        url: 'https://via.placeholder.com/400x400/0066cc/ffffff?text=eBay+Listing',
                        isPrimary: true,
                        order: 1
                      });
                    }
                    
                    console.log(`🖼️ Extracted ${images.length} images for item ${itemId}:`, images.map(img => img.url).slice(0, 3));
                    
                    return {
                      itemId: itemId,
                      title: title.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>'),
                      shortDescription: comprehensiveDescription,
                      description: fullDescription || subtitle || `Active eBay listing: ${title.substring(0, 100)}...`,
                      ...
                      mpn: itemId,
                      image: {
                        imageUrl: images[0]?.url || images[0] // Support both new and old format
                      },
                      images: images, // Array of image objects with url, isPrimary, order
                      allImages: images, // All images for editing interface
                      primaryImageUrl: images[0]?.url || images[0],
                    ...
                    console.log(`❌ GetItem API failed for ${itemId}:`, itemResponse.status);
                    return null;
                  }
                } catch (itemError) {
              ...
              if (validItems.length > 0) {
                const transformedListings = {
                  ...
                  limit: parseInt(limit),
                  debug: {
                    message: 'Successfully fetched eBay listings with full descriptions!',
                    source: 'Trading API - GetMyeBaySelling + GetItem',
                    itemsFound: validItems.length,
                    itemsWithFullDescriptions: validItems.filter(item => item.hasFullDescription).length
                  }
                };
                
                console.log(`✅ Successfully fetched ${validItems.length} eBay listings with full descriptions`);
                return res.status(200).json(transformedListings);
              }
            } else {
              console.log('❌ No active items found in GetMyeBaySelling response');
            }
          }
        } else {
          console.log('❌ GetMyeBaySelling API failed:', listingsResponse.status, listingsResponse.statusText);
          const errorText = await listingsResponse.text();
          console.log('GetMyeBaySelling API error response:', errorText.substring(0, 500));
        }
      } catch (tradingError) {
...
function enhanceEbayImageQuality(url) {
  ...
  if (url.includes('$_')) {
    ...
    console.log(`🖼️ Enhanced image quality: ${url.substring(url.length - 20)} → ${highQualityUrl.substring(highQualityUrl.length - 20)}`);
    return highQualityUrl;
  }
  
  // If URL doesn't have quality parameter, add high-quality one
  if (url.includes('i.ebayimg.com') && !url.includes('set_id')) {
    const enhancedUrl = url + '?set_id=8800005007'; // High quality set
    console.log(`🖼️ Added quality parameter: ${url.substring(url.length - 30)} → enhanced`);
    return enhancedUrl;
  }
